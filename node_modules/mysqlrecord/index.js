...
MysqlRecord.$has_many = function(foreign_table,foreign_key,columns){
	...
	if( !columns ){
		columns = foreign_key;
		foreign_key = this.$table_name.single + '_id';
	}

	foreign_key = foreign_key || this.$table_name.single + '_id';
	...
	this.prototype[foreign_table] = function( _columns , conditions , callback ){

		_columns = utils.format_columns( _columns ) || columns;

		var _conditions = utils.format_conditions( conditions , { start_with : 'and' } );
...
MysqlRecord.$has_and_belongs_to_many = function( foreign_table , columns , options ){
	...
	this.prototype[plural] = function( _columns , condition , callback ){
		...
		var _condition = utils.format_conditions( condition , { start_with : 'and' } );
...
MysqlRecord.$find = function( condition , columns , others , options , callback ){
	...
	var _condition = utils.format_conditions( condition , { table : this.$table_name.plural } );

	var sql = [];
	
	if( options.$with ){

		var _columns = utils.format_columns( columns );
		...
		if( options.pagination ){
			pagination_sql =  ' limit ' + options.pagination.count + ' offset ' + options.pagination.offset + ' ';
		}

		sql.unshift( 'select ' + columns.join(',') + ' from '+
			'(select ' + _columns + ' from ' + this.$table_name.plural + ' where ' + _condition.str + ' ' + others + ' ' + pagination_sql + ') '+
			'as ' + this.$table_name.plural );
		...
		mysql.once( 'end' , function( data ){
			...
			if(!data.length) {
				if( options.pagination ){
					return e.emit('end',{ data:data, total:0 });
				}
				return e.emit( 'end' , data );
			}

			var result = [] , last_data = {};

			data.forEach(function(d){

				var d = group_by_table(d),

					self_data = d[self.$table_name.plural], _last_data = result[result.length - 1] || {},

					same = Object.keys(self_data).every(function(k){
						if(self_data[k] && self_data[k].getTime){
							return self_data[k].getTime() === last_data[k].getTime();
						}
				...
				if(!same){
					...
					last_data = {};
					Object.keys(self_data).forEach(function(s){
						last_data[s] = self_data[s];
					});
					_last_data = self_data;
				}
				...
				Object.keys( d ).forEach( function( k ){
					...
					if( self.$owns[k] || self.$owns_and_owned_by[k] ){
						...
						_last_data[k] = _last_data[k] || [];

						all_null || _last_data[k].push(d[k]);

					}else if(self.$owned_by[s]){

						all_null || (_last_data[s] = d[k]);
			...
			if( options.pagination ){
				mysql.query('select count(*) from '+ self.$table_name.plural + ' where ' + _condition.str + ' ' + others , _condition.data , function(err,data){
					result = {
						data:result,
						total:data[0]['count(*)']
					};
...
MysqlRecord.prototype.$save = function( columns , opt , callback ){
	...
	columns.forEach(function(c){
		var col = self.constructor.__columns[c];
		if( col && col.filters && col.filters.before_save ){
			self[c] = col.filters.before_save(self[c]);
		}
	});
...
MysqlRecord.prototype.$update = function( columns , conditions , return_columns , callback ){
	...
	var data = columns.map( function(c){
		...
		try{
			self[c] = self.constructor.$columns[c].filters.before_update(self[c]);
		}catch(e){
...
MysqlRecord.prototype.$add = function( columns , return_columns , callback ){
	...
	if( typeof return_columns == 'function' ){
		callback = return_columns;
		return_columns = columns.join(',');
	}
	return_columns = return_columns || columns.join(',');
	...
	var data = columns.map(function(c){
		...
		try{
			self[c] = self.constructor.$columns[c].filters.before_create(self[c]);
		}catch(e){
	...
	mysql.query( sql , data , function( err , data ){
		...
		sql = 'select ' + return_columns + ' from ' + table + ' where ' + (self.constructor.$primary_key || 'id') + '=?;';
...
MysqlRecord.prototype.$delete = function( conditions , callback ){
	...
	var self = this
		...
		, sql = 'delete from ' + this.constructor.$table_name.plural + ' where ' + primary_key + '=? ' + conditions.str
		, data = [this[primary_key]].concat(conditions.data);
